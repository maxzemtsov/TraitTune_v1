    ...
    <div className={`flex flex-col h-screen items-center p-4 md:p-0 ${theme === "dark" ? "dark bg-background text-foreground" : "bg-background text-foreground"}`}>
      ...
      <Dialog open={showRegistrationModal} onOpenChange={setShowRegistrationModal}>
        <DialogContent className="sm:max-w-[425px]">
          <DialogHeader><DialogTitle>{t("onboarding.registration_prompt_title")}</DialogTitle><DialogDescription>{t("onboarding.registration_prompt_description_p1", { name: userName || t("user") })}<br/><br/>{t("onboarding.registration_prompt_description_p2")}</DialogDescription></DialogHeader>
          <div className="grid gap-4 py-4">
            <Button onClick={() => handleOAuthSignIn("google")} variant="outline">{t("auth.sign_in_with_google")}</Button>
            <Button onClick={() => handleOAuthSignIn("linkedin")} variant="outline">{t("auth.sign_in_with_linkedin")}</Button>
          </div>
          <DialogFooter><Button variant="ghost" onClick={() => {setShowRegistrationModal(false); fetchAndDisplayNextQuestion(); setAssessmentPhase("assessment_questions");}}>{t("common.later")}</Button></DialogFooter>
        </DialogContent>
      </Dialog>
    </div>
  );
